...
Cypress.Commands.add('setupPage', (blockName, template) => {
    ...
    cy.get('h1.editor-post-title__input[contenteditable="true"]')
        .should('be.visible')
        .click()
        .type(`Block Testing for ${blockName} - ${template}`, { force: true });
...
Cypress.Commands.add('addBlock', (blockName) => {
    ...
    cy.get('.block-editor-block-types-list__list-item button').first().click({ force: true });
    cy.wait(1000);
...
Cypress.Commands.add('publishPage', () => {
    ...
    cy.get('div.components-snackbar', { timeout: 10000 })
        .should('be.visible')
        .within(() => {
            cy.get('.components-snackbar__content a').should('be.visible').click({ force: true });
        });
...
Cypress.Commands.add('deletePage', () => {
    // Click the Save button
    cy.get('button.editor-post-trash').click({ force: true });

    // Wait for the snackbar and click "View Page"
    cy.get('div.components-modal__content', { timeout: 10000 }).within(() => {
        cy.get('button.components-button.is-next-40px-default-size.is-primary').click({
            force: true,
        });
    });
